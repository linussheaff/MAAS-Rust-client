...
pub mod client{
    ...
    impl MaasClient {
        ...
        pub async fn post(&self, endpoint: &str, body: Option<Value>) -> anyhow::Result<serde_json::Value> {
            self.request("POST", endpoint, body).await
        }

        pub async fn put(&self, endpoint: &str, body: Option<Value>) -> anyhow::Result<serde_json::Value> {
            self.request("PUT", endpoint, body).await
        }

        pub async fn delete(&self, endpoint: &str) -> anyhow::Result<serde_json::Value> {
            self.request("DELETE", endpoint, None).await
        }

        ///Performs HTTP requests to MAAS API
        async fn request(&self, method: &str, endpoint: &str, body: Option<Value>) -> anyhow::Result<Value> {
            ...
            let mut request = self.client.request(
                method.parse().context("Invalid HTTP request")?,
                &url,
            );
            request = request
            ...
            if !status.is_success() {
                ...
                return Err(anyhow::anyhow!("MASS API error {}: {}", status, text));
            }
